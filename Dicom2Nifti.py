...
def dicom2nii(dirname):
    ...
    files = find_dicom_files(dirname)
    if not files:
    ...
    vol, pixdim, matAff = dicom_to_volume(series)

    # convert DICOM coords to NIFTI coords (in-place)
    convert_coords(vol, matAff)
    ...
    output = nib.Nifti1Image(vol.T, matAff)
