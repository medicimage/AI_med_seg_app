...
#from subpixel_gh import SubPixelUpscaling
...
def DenseNet(input_shape=None, depth=40, nb_dense_block=3, growth_rate=12, nb_filter=-1, nb_layers_per_block=-1,
             ...
             classes=None, activation='softmax'):
    ...
    '''Instantiate the DenseNet FCN architecture.
        ...
        # Arguments
            ...
            batchsize: Fixed batch size. This is a temporary requirement for
                computation of output shape in the case of Deconvolution3D layers.
...
def DenseNetT(input_shape, nb_dense_block=5, growth_rate=16, nb_layers_per_block=4,
              reduction=0.0, dropout_rate=0.0, weight_decay=1e-4, init_conv_filters=48,
              include_top=True, weights=None, input_tensor=None, classes=1, activation='softmax',
              upsampling_conv=128, upsampling_type='deconv'):
    '''Instantiate the DenseNet FCN architecture.
    ...
    x = __create_fcn_dense_net_t(classes, img_input, include_top, nb_dense_block,
                                 growth_rate, reduction, dropout_rate, weight_decay,
                                 nb_layers_per_block, upsampling_conv, upsampling_type,
                                 init_conv_filters, input_shape, activation)
    ...
    if input_tensor is not None:
        ...
        inputs = img_input
    # Create model.
    model = Model(inputs, x, name='fcn-densenet-t')

    return model


def DenseNetFCN103(input_shape, nb_dense_block=5, growth_rate=16, nb_layers_per_block=4,
                   reduction=0.0, dropout_rate=0.0, weight_decay=1e-4, init_conv_filters=48,
                   include_top=True, weights=None, input_tensor=None, classes=1, activation='softmax',
                   upsampling_conv=128, upsampling_type='deconv'):
    '''Instantiate the DenseNet FCN architecture.
    ...
    x = __create_fcn_dense_net103(classes, img_input, include_top, nb_dense_block,
                                  growth_rate, reduction, dropout_rate, weight_decay,
                                  nb_layers_per_block, upsampling_conv, upsampling_type,
                                  init_conv_filters, input_shape, activation)
...
def __dense_block(x, nb_layers, nb_filter, growth_rate, bottleneck=True, dropout_rate=None, weight_decay=1e-4,
...
def __create_fcn_dense_net(nb_classes, img_input, include_top, nb_dense_block=5, growth_rate=12,
                           reduction=0.0, dropout_rate=None, weight_decay=1e-8,
    ...
    if concat_axis == 1:  # channels_first dim ordering
        _, rows, cols, cols2 = input_shape
    else:
        rows, cols, cols2, _ = input_shape
    ...
    # Initial convolution
    # x = Conv3D(init_conv_filters, (7, 7,7), kernel_initializer='he_normal', padding='same', name='initial_conv3D',
    #     use_bias=False, kernel_regularizer=l2(weight_decay))(img_input)
    x = Conv3D(init_conv_filters, (3, 3, 3), kernel_initializer='he_normal', padding='same', strides=(2, 2, 2),
               name='initial_conv3D',
               use_bias=False, kernel_regularizer=l2(weight_decay))(img_input)
    x = Conv3D(init_conv_filters, (3, 3, 3), kernel_initializer='he_normal', padding='same', name='initial_conv3D',
               use_bias=False, dilation_rate=1, kernel_regularizer=l2(weight_decay))(x)
    x = BatchNormalization(axis=concat_axis, epsilon=1.1e-5)(x)
    x = Activation('relu')(x)

    x = Conv3D(init_conv_filters, (3, 3, 3), kernel_initializer='he_normal', padding='same', name='second_conv3D',
               use_bias=False, dilation_rate=1, kernel_regularizer=l2(weight_decay))(x)
    x = BatchNormalization(axis=concat_axis, epsilon=1.1e-5)(x)
    x = Activation('relu')(x)
    # x = BatchNormalization(axis=concat_axis, epsilon=1.1e-5)(x)
    # x = Activation('relu')(x)
    x = Conv3D(init_conv_filters, (3, 3, 3), kernel_initializer='he_normal', padding='same', name='third_conv3D',
               use_bias=False, dilation_rate=1, kernel_regularizer=l2(weight_decay))(x)
    ...
    for block_idx in range(nb_dense_block):
        ...
        x_up = Conv3DTranspose(init_conv_filters, (3, 3, 3), activation='relu', padding='same', strides=(2, 2, 2),
                               kernel_initializer='he_normal', kernel_regularizer=l2(weight_decay))(x_up)

    if include_top:
        x = Conv3D(nb_classes, (1, 1, 1), activation='linear', padding='same', use_bias=False)(x_up)

        if K.image_data_format() == 'channels_first':
            channel, row, col, col2 = input_shape
        else:
            row, col, col2, channel = input_shape

        x = Reshape((row * col * col2, nb_classes))(x)
        x = Activation(activation)(x)
        x = Reshape((row, col, col2, nb_classes), name='seg')(x)
    else:
        x = x_up
...
def __create_fcn_dense_net_t(nb_classes, img_input, include_top, nb_dense_block=5, growth_rate=12,
                             reduction=0.0, dropout_rate=None, weight_decay=1e-8,
                             nb_layers_per_block=4, nb_upsampling_conv=128, upsampling_type='upsampling',
                             init_conv_filters=48, input_shape=None, activation='deconv'):
    ''' Build the DenseNet model
    ...
    # x = BatchNormalization(axis=concat_axis, epsilon=1.1e-5)(x)
    # x = Activation('relu')(x)
    x = Conv3D(init_conv_filters, (3, 3, 3), kernel_initializer='he_normal', padding='same', name='fourth_conv3D',
               use_bias=False, dilation_rate=1, kernel_regularizer=l2(weight_decay))(x)
    ...
    # The last dense_block does not have a transition_down_block
    # return the concatenated feature maps without the concatenation of the input
    _, nb_filter, concat_list = __dense_block(x, 15, nb_filter, growth_rate,
                                              dropout_rate=dropout_rate, weight_decay=weight_decay,
                                              return_concat_list=True)
...
def __create_fcn_dense_net103(nb_classes, img_input, include_top=True, nb_dense_block=5, growth_rate=12,
    ...
    if concat_axis == 1:  # channels_first dim ordering
        ...
        rows, cols, slices, _ = input_shape
    ...
    if type(nb_layers_per_block) is list or type(nb_layers_per_block) is tuple:
        ...
        # nb_layers = [nb_layers_per_block] * (2 * nb_dense_block + 1)
        nb_layers = [4, 5, 7, 10, 12, 15, 12, 10, 7, 5, 4];
        # bottleneck_nb_layers = nb_layers;

    # compute compression factor
    compression = 1.0 - reduction

    # Initial convolution
    x = Conv3D(init_conv_filters, (3, 3, 3), activation='relu', padding='same', strides=(2, 2, 2),
    ...
    if include_top:
        ...
        if K.image_data_format() == 'channels_first':
            ...
            row, col, slc, channel = input_shape

        x = Reshape((row * col * slc, nb_classes))(x)
        x = Activation(activation)(x)
        x = Reshape((row, col, slc, nb_classes))(x)
    else:
        x = x_up
