...
def get_unet2(xyDim, zDim, n_channels, n_classes, deepRed, multiHead):
    ...
    if multiHead:
        ...
        model.compile(optimizer=Adam(lr=1e-3), loss={'seg': dice_coef_loss2, 'box': dice_coef_loss2},
                      loss_weights=[1., 1.])
...
def get_rbunet(xyDim, zDim, n_channels, n_classes, deepRed, multiHead):
    ...
    if multiHead:
        ...
        model = Model(inputs=[inputs], outputs=[segPred, boxPred])
        model.compile(optimizer=Adam(lr=1e-3, decay=0.001), loss={'seg': dice_coef_loss2, 'box': dice_coef_loss2},
                      loss_weights=[1., 1.])
...
def get_meshNet(xyDim, zDim, n_channels, n_classes, deepRed, multiHead):
    ...
    for i in range(n_layers):
        if i != n_layers:
            net.add(Conv3D(output[i], (kZ[i], kY[i], kX[i]), input_shape=(zDim, xyDim, xyDim, inputt[i]),
                           dilation_rate=dilZ[i]));
...
def augmentation(Data, Labels, scaleRange, steps):
    ...
    labAug = np.zeros(
        (len(scaleVec) * Labels.shape[0], Labels.shape[1], Labels.shape[2], Labels.shape[3], Labels.shape[4]));
...
def augment_sample(image, label, labelb):
    ...
    if diffSize < 0:
        n = int(abs(diffSize) / 2);
        DataAug = np.pad(DataS1, ((n, abs(diffSize) - n), (n, abs(diffSize) - n), (n, abs(diffSize) - n), (0, 0)),
                         'edge');
        labAug = np.pad(labelS1, ((n, abs(diffSize) - n), (n, abs(diffSize) - n), (n, abs(diffSize) - n), (0, 0)),
                        'edge');
        labAugb = np.pad(labelS1b, ((n, abs(diffSize) - n), (n, abs(diffSize) - n), (n, abs(diffSize) - n), (0, 0)),
                         'edge');
    elif diffSize >= 0:
        n = int(abs(diffSize) / 2);
...
def IoU3D(boxPred0, boxTest0, labelsPred0):
    ...
    intersRight = (min([rbt[0] + int(rbt[3] / 2), rbp[0] + int(rbp[3] / 2)]) - max(
        [rbt[0] - int(rbt[3] / 2), rbp[0] - int(rbp[3] / 2)])) * \
                  (min([rbt[1] + int(rbt[4] / 2), rbp[1] + int(rbp[4] / 2)]) - max(
                      [rbt[1] - int(rbt[4] / 2), rbp[1] - int(rbp[4] / 2)])) * \
                  (min([rbt[2] + int(rbt[5] / 2), rbp[2] + int(rbp[5] / 2)]) - max(
                      [rbt[2] - int(rbt[5] / 2), rbp[2] - int(rbp[5] / 2)]))

    unionRight = (rbt[3] * rbt[4] * rbt[5] + rbp[3] * rbp[4] * rbp[5]) - intersRight;

    IoUright = intersRight / unionRight;

    intersLeft = (min([lbt[0] + int(lbt[3] / 2), lbp[0] + int(lbp[3] / 2)]) - max(
        [lbt[0] - int(lbt[3] / 2), lbp[0] - int(lbp[3] / 2)])) * \
                 (min([lbt[1] + int(lbt[4] / 2), lbp[1] + int(lbp[4] / 2)]) - max(
                     [lbt[1] - int(lbt[4] / 2), lbp[1] - int(lbp[4] / 2)])) * \
                 (min([lbt[2] + int(lbt[5] / 2), lbp[2] + int(lbp[5] / 2)]) - max(
                     [lbt[2] - int(lbt[5] / 2), lbp[2] - int(lbp[5] / 2)]))
    ...
    labelsPredRight[labelsPredRight != 1] = 0;
    labelsPredRight[int(rbt[0] - (rbt[3] / 2)):int(rbt[0] + (rbt[3] / 2)),
    int(rbt[1] - int(rbt[4] / 2)):int(rbt[1] + (rbt[4] / 2)),
    int(rbt[2] - (rbt[5] / 2)):int(rbt[2] + (rbt[5] / 2))] = 0;
    missedVoxelsRight = np.count_nonzero(labelsPredRight);

    labelsPredLeft = np.copy(labelsPred0);
    labelsPredLeft[labelsPredLeft != 2] = 0;
    labelsPredLeft[int(lbt[0] - (lbt[3] / 2)):int(lbt[0] + (lbt[3] / 2)),
    int(lbt[1] - int(lbt[4] / 2)):int(lbt[1] + (lbt[4] / 2)),
    int(lbt[2] - (lbt[5] / 2)):int(lbt[2] + (lbt[5] / 2))] = 0;
